...
def task2_energy_vs_temperature():
    ...
    V = N / rho
    L = np.sqrt(V)
    temperatures = np.linspace(0.1, 1.2, 12)
    energies = []

    print("--- ЗАДАЧА 2: E(T) при ρ = 0.4 ---")
    for T0 in temperatures:
        summary, _, _ = simulate_with_gr_return_gr(N=N, L=L, T0=T0)
        E_mean = summary["mean_E"] / N
        energies.append(E_mean)
        print(f"T = {T0:.2f} | E/N = {E_mean:.4f}")

    # --- Построение графика ---
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=temperatures, y=energies, mode='lines+markers', 
                             marker=dict(color='orange'), name='E/N'))
    fig.update_layout(
        xaxis_title="Температура T (безразмерная)",
        yaxis_title="Удельная энергия E/N (в ε)",
        title="E(T) при плотности ρ = 0.4",
        template="plotly_dark",
        legend_title="Величина"
    )
    fig.show()
